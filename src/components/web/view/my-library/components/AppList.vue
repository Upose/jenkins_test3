 ...
 * @LastEditors: huyu
 * @LastEditTime: 2023-01-09 17:08:36
 * @Description: 
-->
<template>
  <div class="apply">
    ...
    <div class="app-content">
      <div class="item-box">
        <template v-for="(item,index) in appData.appList">
          <a :href="item.frontUrl" target="_blank" class="app-item" :key="item.appId" v-if="index<6">
            <div class="app">
        ...
        <!--加载中-->
        <div class="temp-loading" v-if="listLoading"></div>
      </div>
      <a :href="appData.myAppRouteCode" target="_blank" class="right-arr">
        <span>更多</span>
...
.apply {
  ...
  .app-title-box {
    ...
    .app-title-tag {
      background: url("~@/assets/web/img/personal/icon-right.png") no-repeat;
      position: absolute;
      top: 60px;
      left: 50px;
      width: 6px;
...
.chance {
  ...
  .chance-app {
    ...
    .app-select {
      ...
      color: #458dda;
      position: relative;
      background: url("~@/assets/web/img/active-bottom.png") no-repeat right
        bottom;
