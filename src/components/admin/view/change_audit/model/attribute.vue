    ...
    <div class="t-p">
      <el-table v-loading="loading" stripe ref="singleTable" :data="tableData" @selection-change="handleSelectionApp" border class="admin-table" :header-cell-style="{background:'#F1F3F7'}">
        ...
        <el-table-column prop="content" label="操作" fixed="right" width="160" align="center">
          <template slot-scope="scope">
            <el-button @click="handleAudit(scope.row)" type="text" size="mini" icon="iconfont el-icon-vip-bianji" round v-if="scope.row.status == 0" v-has="'approve:propertyApprove'">审核</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="content" label="详细日志" width="160" align="center">
          <template slot-scope="scope">
            <el-button @click="handleLog(scope.row)" type="text" size="mini" icon="iconfont el-icon-vip-yulan" round v-has="'approve:propertyDetail'">详细记录</el-button>
          </template>
        </el-table-column>
      </el-table>
