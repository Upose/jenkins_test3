<template>
  <div>
    ...
    <div class="t-p">
      <el-table v-loading="loading" stripe ref="singleTable" :data="isAuth('approve:propertyList')?tableData:[]" @selection-change="handleSelectionApp" border class="admin-table" :header-cell-style="{background:'#F1F3F7'}">
        ...
        <el-table-column prop="content" label="详细日志" width="160" align="center">
          <template slot-scope="scope">
            <el-button @click="handleLog(scope.row)" type="text" size="mini" icon="iconfont el-icon-vip-yulan" round v-if="isAuth('approve:propertyDetail')">详细记录</el-button>
          </template>
        </el-table-column>
      </el-table>
