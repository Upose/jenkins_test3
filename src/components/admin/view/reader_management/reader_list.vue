            ...
            <div class="t-p">
              <el-table @selection-change="handleSelectionChange" v-if="dataKey" ref="singleTable" stripe :data="isAuth('reader:list')?tableData:[]" border :header-cell-style="{background:'#F1F3F7'}" class="admin-table">
                ...
                <el-table-column prop="content" label="操作" width="260">
                  <template slot-scope="scope">
                    <el-button @click="handleDel(scope.row)" type="text" size="mini" icon="el-icon-vip-shanchu-1" class="operate-red-btn" round v-if="isAuth('reader:delete')">删除</el-button>
                    <el-button @click="handleSet(scope.row)" type="text" size="mini" icon="el-icon-vip-yulan" round v-if="isAuth('reader:update')">查看</el-button>
                  </template>
                </el-table-column>
              </el-table>
